/* ===== 23e8a0230 workers.fishing.trigger_monitor:279 ===== */
/* ghidra_name=FUN_23e8a0230 entry=23e8a0230 size=3062 */

/* WARNING: Globals starting with '_' overlap smaller symbols at the same address */

longlong * FUN_23e8a0230(longlong param_1,undefined8 param_2,undefined8 *param_3)

{
  longlong *plVar1;
  code *pcVar2;
  longlong lVar3;
  longlong *plVar4;
  undefined8 *puVar5;
  code *pcVar6;
  char cVar7;
  int iVar8;
  longlong *plVar9;
  longlong *plVar10;
  code *pcVar11;
  longlong *plVar12;
  longlong *plVar13;
  longlong *plVar14;
  longlong lVar15;
  undefined4 uVar16;
  undefined8 uVar17;
  undefined8 uVar18;
  code *local_88;
  longlong *plStack_80;
  undefined1 local_78 [16];
  longlong *local_68;
  
  plVar9 = _DAT_23eede460;
  plVar1 = (longlong *)*param_3;
  pcVar2 = (code *)param_3[1];
  local_68 = (longlong *)0x0;
  local_78._0_8_ = 0;
  local_78._8_8_ = 0;
  if (_DAT_23eede460 == (longlong *)0x0) {
LAB_23e8a028b:
    _DAT_23eede460 = (longlong *)FUN_23e91f3b0(param_1,_DAT_23eede458,DAT_23eede5a0,0x30);
  }
  else {
    lVar15 = *_DAT_23eede460;
    if (1 < lVar15) {
      *_DAT_23eede460 = lVar15 + -1;
      goto LAB_23e8a028b;
    }
    if (_DAT_23eede460[2] != 0) {
      *_DAT_23eede460 = lVar15 + -1;
      if (lVar15 + -1 == 0) {
        (**(code **)(plVar9[1] + 0x30))(plVar9);
      }
      goto LAB_23e8a028b;
    }
  }
  plVar4 = _DAT_23eede460;
  lVar15 = *(longlong *)(param_1 + 0x38);
  plVar9 = _DAT_23eede460 + 9;
  lVar3 = *(longlong *)(lVar15 + 8);
  _DAT_23eede460[0xf] = lVar3;
  *(longlong **)(lVar15 + 8) = plVar9;
  if ((lVar3 != 0) &&
     (((*(char *)(lVar3 + 0x45) == '\x01' ||
       ((ulonglong)
        (*(longlong *)(lVar3 + 0x20) + 0xb8 +
        (longlong)*(int *)(*(longlong *)(lVar3 + 0x20) + 0xa8) * 2) <= *(ulonglong *)(lVar3 + 0x38))
       ) && (plVar4[0xe] != 0)))) {
    plVar9 = *(longlong **)(lVar3 + 0x28);
    *(longlong **)(plVar4[0xe] + 0x10) = plVar9;
    if (plVar9 != (longlong *)0x0) {
      *plVar9 = *plVar9 + 1;
    }
  }
  *plVar4 = *plVar4 + 1;
  uVar17 = _DAT_23eedc270;
  *(undefined4 *)(plVar4 + 8) = 0;
  plVar9 = (longlong *)FUN_23e8bc2f0(plVar1,uVar17);
  if (plVar9 == (longlong *)0x0) {
    local_78._0_8_ = *(undefined8 *)(param_1 + 0x60);
    plVar9 = *(longlong **)(param_1 + 0x70);
    plVar13 = (longlong *)0x0;
    plVar10 = (longlong *)0x0;
    *(undefined8 *)(param_1 + 0x70) = 0;
    pcVar11 = (code *)0x0;
    uVar16 = 0x119;
    *(undefined1 (*) [16])(param_1 + 0x60) = (undefined1  [16])0x0;
    plVar12 = (longlong *)0x0;
    local_78._8_8_ = *(undefined8 *)(param_1 + 0x68);
    plVar14 = DAT_23ed6a4f8;
    goto joined_r0x00023e8a0a73;
  }
  plVar10 = (longlong *)FUN_23e8bc2f0(plVar1,_DAT_23eedc088);
  if (plVar10 == (longlong *)0x0) {
    local_78._0_8_ = *(undefined8 *)(param_1 + 0x60);
    local_78._8_8_ = *(undefined8 *)(param_1 + 0x68);
    local_68 = *(longlong **)(param_1 + 0x70);
    *(undefined8 *)(param_1 + 0x70) = 0;
    *(undefined1 (*) [16])(param_1 + 0x60) = (undefined1  [16])0x0;
    *plVar9 = *plVar9 + -1;
    plVar10 = (longlong *)0x0;
    if (*plVar9 == 0) {
      plVar13 = (longlong *)0x0;
      plVar12 = (longlong *)0x0;
      pcVar11 = (code *)0x0;
      (**(code **)(plVar9[1] + 0x30))(plVar9);
      uVar16 = 0x119;
      plVar9 = local_68;
      plVar14 = DAT_23ed6a4f8;
    }
    else {
      plVar13 = (longlong *)0x0;
      plVar12 = (longlong *)0x0;
      pcVar11 = (code *)0x0;
      uVar16 = 0x119;
      plVar9 = local_68;
      plVar14 = DAT_23ed6a4f8;
    }
    goto joined_r0x00023e8a0a73;
  }
  *(undefined4 *)(plVar4 + 5) = 0x119;
  local_88 = pcVar2;
  plStack_80 = plVar10;
  pcVar11 = (code *)FUN_23e94ed00(param_1,plVar9,&local_88);
  *plVar9 = *plVar9 + -1;
  if (*plVar9 == 0) {
    (**(code **)(plVar9[1] + 0x30))(plVar9);
  }
  *plVar10 = *plVar10 + -1;
  if (*plVar10 == 0) {
    (**(code **)(plVar10[1] + 0x30))(plVar10);
  }
  pcVar6 = _Py_NoneStruct_exref;
  lVar15 = DAT_23ed6cce8;
  if (pcVar11 == (code *)0x0) {
    local_78._0_8_ = *(undefined8 *)(param_1 + 0x60);
    local_78._8_8_ = *(undefined8 *)(param_1 + 0x68);
    plVar9 = *(longlong **)(param_1 + 0x70);
    plVar13 = (longlong *)0x0;
    plVar12 = (longlong *)0x0;
    *(undefined8 *)(param_1 + 0x70) = 0;
    plVar10 = (longlong *)0x0;
    uVar16 = 0x119;
    *(undefined1 (*) [16])(param_1 + 0x60) = (undefined1  [16])0x0;
    plVar14 = DAT_23ed6a4f8;
    goto joined_r0x00023e8a0a73;
  }
  lVar3 = *(longlong *)(param_1 + 0x10);
  iVar8 = *(int *)(lVar3 + 0x1410);
  plVar9 = *(longlong **)(DAT_23ed6cce8 + 0x20);
  *plVar9 = *plVar9 + 1;
  if (pcVar11 == pcVar6) {
    if (iVar8 == 0) {
      plVar10 = (longlong *)FUN_23a33a530(PyDict_Type_exref);
      plVar9 = *(longlong **)(lVar15 + 0x20);
    }
    else {
      *(int *)(lVar3 + 0x1410) = iVar8 + -1;
      plVar10 = *(longlong **)(lVar3 + 0x1190 + (longlong)(iVar8 + -1) * 8);
      *plVar10 = 1;
    }
    lVar15 = *(longlong *)(lVar15 + 0x28);
    plVar10[2] = 0;
    plVar10[4] = (longlong)plVar9;
    plVar10[5] = lVar15;
    lVar15 = *(longlong *)(param_1 + 0x38);
    plVar10[3] = 1;
    lVar3 = *(longlong *)(lVar15 + 8);
    plVar9 = *(longlong **)(lVar3 + 0x28);
    plVar4 = (longlong *)plVar9[2];
    *(undefined8 *)(lVar15 + 8) = *(undefined8 *)(lVar3 + 0x30);
    *(undefined4 *)(plVar9 + 8) = 0xffffffff;
    if (plVar4 != (longlong *)0x0) {
      plVar9[2] = 0;
      *plVar4 = *plVar4 + -1;
      if (*plVar4 == 0) {
        (**(code **)(plVar4[1] + 0x30))();
      }
    }
    *plVar9 = *plVar9 + -1;
    if (*plVar9 == 0) {
      (**(code **)(plVar9[1] + 0x30))(plVar9);
    }
    *(longlong *)pcVar6 = *(longlong *)pcVar6 + -1;
    lVar15 = *(longlong *)pcVar6;
    plVar9[0xf] = 0;
    if (lVar15 == 0) {
      (**(code **)(*(longlong *)(pcVar6 + 8) + 0x30))(pcVar6);
    }
LAB_23e8a0766:
    *plVar1 = *plVar1 + -1;
    if (*plVar1 == 0) {
      (**(code **)(plVar1[1] + 0x30))(plVar1);
    }
    *(longlong *)pcVar2 = *(longlong *)pcVar2 + -1;
    if (*(longlong *)pcVar2 != 0) {
      return plVar10;
    }
    (**(code **)(*(longlong *)(pcVar2 + 8) + 0x30))(pcVar2);
    return plVar10;
  }
  if (iVar8 == 0) {
    plVar10 = (longlong *)FUN_23a33a530(PyDict_Type_exref);
    plVar9 = *(longlong **)(lVar15 + 0x20);
  }
  else {
    *(int *)(lVar3 + 0x1410) = iVar8 + -1;
    plVar10 = *(longlong **)(lVar3 + 0x1190 + (longlong)(iVar8 + -1) * 8);
    *plVar10 = 1;
  }
  lVar15 = *(longlong *)(lVar15 + 0x28);
  plVar10[2] = 0;
  plVar10[3] = 1;
  plVar10[4] = (longlong)plVar9;
  plVar10[5] = lVar15;
  plVar12 = (longlong *)FUN_23e8bc2f0(plVar1,_DAT_23eedc278);
  if (plVar12 == (longlong *)0x0) goto LAB_23e8a07e0;
  plVar9 = (longlong *)FUN_23e8bc2f0(plVar1,_DAT_23eedc0f0);
  if (plVar9 == (longlong *)0x0) {
LAB_23e8a0ba0:
    local_78._0_8_ = *(undefined8 *)(param_1 + 0x60);
    local_78._8_8_ = *(undefined8 *)(param_1 + 0x68);
    local_68 = *(longlong **)(param_1 + 0x70);
    *(undefined8 *)(param_1 + 0x70) = 0;
    *(undefined1 (*) [16])(param_1 + 0x60) = (undefined1  [16])0x0;
    *plVar12 = *plVar12 + -1;
    plVar9 = local_68;
    uVar17 = local_78._0_8_;
    uVar18 = local_78._8_8_;
    if (*plVar12 == 0) {
      (**(code **)(plVar12[1] + 0x30))(plVar12);
      plVar9 = local_68;
      uVar17 = local_78._0_8_;
      uVar18 = local_78._8_8_;
    }
  }
  else {
    plVar13 = (longlong *)FUN_23a38cc10(param_1,plVar9,_DAT_23eedc198);
    *plVar9 = *plVar9 + -1;
    if (*plVar9 == 0) {
      (**(code **)(plVar9[1] + 0x30))(plVar9);
    }
    if (plVar13 == (longlong *)0x0) goto LAB_23e8a0ba0;
    *(undefined4 *)(plVar4 + 5) = 0x120;
    local_88 = pcVar11;
    plStack_80 = plVar13;
    plVar9 = (longlong *)FUN_23e94ed00(param_1,plVar12,&local_88);
    *plVar12 = *plVar12 + -1;
    if (*plVar12 == 0) {
      (**(code **)(plVar12[1] + 0x30))(plVar12);
    }
    *plVar13 = *plVar13 + -1;
    if (*plVar13 == 0) {
      (**(code **)(plVar13[1] + 0x30))(plVar13);
    }
    if (plVar9 == (longlong *)0x0) {
LAB_23e8a07e0:
      plVar9 = *(longlong **)(param_1 + 0x70);
      *(undefined8 *)(param_1 + 0x70) = 0;
      uVar17 = *(undefined8 *)(param_1 + 0x60);
      *(undefined1 (*) [16])(param_1 + 0x60) = (undefined1  [16])0x0;
      uVar18 = *(undefined8 *)(param_1 + 0x68);
    }
    else {
      plVar14 = (longlong *)FUN_23a388310(plVar9);
      *plVar9 = *plVar9 + -1;
      if (*plVar9 == 0) {
        (**(code **)(plVar9[1] + 0x30))(plVar9);
      }
      if (plVar14 == (longlong *)0x0) goto LAB_23e8a07e0;
      plVar12 = (longlong *)(**(code **)(plVar14[1] + 0xe0))(plVar14);
      if ((plVar12 == (longlong *)0x0) &&
         (plVar12 = (longlong *)FUN_23a3c1b70(param_1,local_78,0,2), plVar12 == (longlong *)0x0)) {
        plVar13 = (longlong *)0x0;
      }
      else {
        plVar13 = (longlong *)(**(code **)(plVar14[1] + 0xe0))(plVar14);
        if (((plVar13 != (longlong *)0x0) ||
            (plVar13 = (longlong *)FUN_23a3c1b70(param_1,local_78,1,2), plVar13 != (longlong *)0x0))
           && (cVar7 = FUN_23a3884a0(param_1,local_78,plVar14,2), cVar7 != '\0')) {
          *plVar14 = *plVar14 + -1;
          if (*plVar14 == 0) {
            (**(code **)(plVar14[1] + 0x30))(plVar14);
          }
          if (*plVar12 == 0) {
            (**(code **)(plVar12[1] + 0x30))(plVar12);
          }
          if (*plVar13 == 0) {
            (**(code **)(plVar13[1] + 0x30))(plVar13);
          }
          lVar15 = FUN_23e8db270();
          if (lVar15 == 0) {
            FUN_23e915740(param_1,local_78,_DAT_23eedc280);
            uVar16 = 0x121;
            plVar9 = local_68;
            plVar14 = DAT_23ed6a4f8;
          }
          else {
            iVar8 = FUN_23e97d0c0(plVar12,lVar15);
            uVar17 = _DAT_23eedc288;
            if (iVar8 != -1) {
              if (iVar8 == 1) {
                plVar9 = (longlong *)_PyDict_NewPresized(2);
                PyDict_SetItem(plVar9,uVar17,plVar12);
                PyDict_SetItem(plVar9,_DAT_23eedc290,plVar13);
                PyDict_SetItem(plVar10,_DAT_23eedc198,plVar9);
                *plVar9 = *plVar9 + -1;
                if (*plVar9 == 0) {
                  (**(code **)(plVar9[1] + 0x30))(plVar9);
                }
              }
              lVar15 = *(longlong *)(*(longlong *)(param_1 + 0x38) + 8);
              plVar9 = *(longlong **)(lVar15 + 0x28);
              plVar4 = (longlong *)plVar9[2];
              *(undefined8 *)(*(longlong *)(param_1 + 0x38) + 8) = *(undefined8 *)(lVar15 + 0x30);
              *(undefined4 *)(plVar9 + 8) = 0xffffffff;
              if (plVar4 != (longlong *)0x0) {
                plVar9[2] = 0;
                *plVar4 = *plVar4 + -1;
                if (*plVar4 == 0) {
                  (**(code **)(plVar4[1] + 0x30))();
                }
              }
              *plVar9 = *plVar9 + -1;
              if (*plVar9 == 0) {
                (**(code **)(plVar9[1] + 0x30))(plVar9);
              }
              plVar9[0xf] = 0;
              *plVar10 = *plVar10 + 1;
              *(longlong *)pcVar11 = *(longlong *)pcVar11 + -1;
              if (*(longlong *)pcVar11 == 0) {
                (**(code **)(*(longlong *)(pcVar11 + 8) + 0x30))(pcVar11);
              }
              *plVar10 = *plVar10 + -1;
              if (*plVar10 == 0) {
                (**(code **)(plVar10[1] + 0x30))(plVar10);
              }
              *plVar12 = *plVar12 + -1;
              if (*plVar12 == 0) {
                (**(code **)(plVar12[1] + 0x30))(plVar12);
              }
              *plVar13 = *plVar13 + -1;
              if (*plVar13 == 0) {
                (**(code **)(plVar13[1] + 0x30))(plVar13);
              }
              goto LAB_23e8a0766;
            }
            local_78._0_8_ = *(undefined8 *)(param_1 + 0x60);
            plVar9 = *(longlong **)(param_1 + 0x70);
            uVar16 = 0x121;
            *(undefined8 *)(param_1 + 0x70) = 0;
            *(undefined1 (*) [16])(param_1 + 0x60) = (undefined1  [16])0x0;
            local_78._8_8_ = *(undefined8 *)(param_1 + 0x68);
            plVar14 = DAT_23ed6a4f8;
          }
          goto joined_r0x00023e8a0a73;
        }
      }
      plVar9 = local_68;
      uVar18 = local_78._8_8_;
      uVar17 = local_78._0_8_;
      local_68 = (longlong *)0x0;
      local_78 = (undefined1  [16])0x0;
      *plVar14 = *plVar14 + -1;
      if (*plVar14 == 0) {
        (**(code **)(plVar14[1] + 0x30))(plVar14);
      }
      local_68 = (longlong *)0x0;
      local_78._0_8_ = 0;
      local_78._8_8_ = 0;
      if ((plVar12 != (longlong *)0x0) && (*plVar12 = *plVar12 + -1, *plVar12 == 0)) {
        (**(code **)(plVar12[1] + 0x30))(plVar12);
      }
      if ((plVar13 != (longlong *)0x0) && (*plVar13 = *plVar13 + -1, *plVar13 == 0)) {
        (**(code **)(plVar13[1] + 0x30))(plVar13);
      }
    }
  }
  uVar16 = 0x120;
  plVar13 = (longlong *)0x0;
  plVar12 = (longlong *)0x0;
  local_78._0_8_ = uVar17;
  local_78._8_8_ = uVar18;
  plVar14 = DAT_23ed6a4f8;
joined_r0x00023e8a0a73:
  DAT_23ed6a4f8 = plVar14;
  local_68 = plVar9;
  if (plVar9 == (longlong *)0x0) {
    if (plVar14 == (longlong *)0x0) {
      plVar14 = (longlong *)FUN_23a33a530(PyTraceBack_Type_exref);
    }
    else {
      DAT_23ed6a4f8 = (longlong *)*plVar14;
      *plVar14 = 1;
      DAT_23ed6a4f0 = DAT_23ed6a4f0 + -1;
    }
    plVar14[2] = 0;
    plVar14[3] = (longlong)plVar4;
    *plVar4 = *plVar4 + 1;
    *(undefined4 *)((longlong)plVar14 + 0x24) = uVar16;
    pcVar6 = _PyRuntime_exref;
    *(undefined4 *)(plVar14 + 4) = 0xffffffff;
    lVar15 = *(longlong *)(*(longlong *)(*(longlong *)(pcVar6 + 0x1f8) + 0x10) + 0x2e8);
    lVar3 = plVar14[-1];
    puVar5 = *(undefined8 **)(lVar15 + 8);
    *puVar5 = plVar14 + -2;
    plVar14[-2] = lVar15;
    plVar14[-1] = (ulonglong)((uint)lVar3 & 3) | (ulonglong)puVar5;
    *(longlong **)(lVar15 + 8) = plVar14 + -2;
    plVar9 = plVar14;
    if ((local_68 != (longlong *)0x0) && (*local_68 = *local_68 + -1, *local_68 == 0)) {
      (**(code **)(local_68[1] + 0x30))(local_68);
    }
  }
  else if ((longlong *)plVar9[3] != plVar4) {
    if (plVar14 == (longlong *)0x0) {
      plVar14 = (longlong *)FUN_23a33a530(PyTraceBack_Type_exref);
    }
    else {
      DAT_23ed6a4f8 = (longlong *)*plVar14;
      *plVar14 = 1;
      DAT_23ed6a4f0 = DAT_23ed6a4f0 + -1;
    }
    plVar14[3] = (longlong)plVar4;
    *plVar4 = *plVar4 + 1;
    *(undefined4 *)((longlong)plVar14 + 0x24) = uVar16;
    pcVar6 = _PyRuntime_exref;
    *(undefined4 *)(plVar14 + 4) = 0xffffffff;
    lVar15 = *(longlong *)(*(longlong *)(*(longlong *)(pcVar6 + 0x1f8) + 0x10) + 0x2e8);
    lVar3 = plVar14[-1];
    puVar5 = *(undefined8 **)(lVar15 + 8);
    *puVar5 = plVar14 + -2;
    plVar14[-2] = lVar15;
    plVar14[-1] = (ulonglong)((uint)lVar3 & 3) | (ulonglong)puVar5;
    *(longlong **)(lVar15 + 8) = plVar14 + -2;
    plVar14[2] = (longlong)plVar9;
    *plVar9 = *plVar9 + 1;
    plVar9 = plVar14;
    if ((local_68 != (longlong *)0x0) && (*local_68 = *local_68 + -1, *local_68 == 0)) {
      (**(code **)(local_68[1] + 0x30))();
    }
  }
  local_68 = plVar9;
  FUN_23e8bba40(plVar4,"oooooo",plVar1,pcVar2,pcVar11,plVar10,plVar12,plVar13);
  if (_DAT_23eede460 == plVar4) {
    *plVar4 = *plVar4 + -1;
    if (*plVar4 == 0) {
      (**(code **)(plVar4[1] + 0x30))(plVar4);
    }
    _DAT_23eede460 = (longlong *)0x0;
  }
  lVar15 = *(longlong *)(*(longlong *)(param_1 + 0x38) + 8);
  plVar9 = *(longlong **)(lVar15 + 0x28);
  plVar4 = (longlong *)plVar9[2];
  *(undefined8 *)(*(longlong *)(param_1 + 0x38) + 8) = *(undefined8 *)(lVar15 + 0x30);
  *(undefined4 *)(plVar9 + 8) = 0xffffffff;
  if (plVar4 != (longlong *)0x0) {
    plVar9[2] = 0;
    *plVar4 = *plVar4 + -1;
    if (*plVar4 == 0) {
      (**(code **)(plVar4[1] + 0x30))();
    }
  }
  *plVar9 = *plVar9 + -1;
  if (*plVar9 == 0) {
    (**(code **)(plVar9[1] + 0x30))(plVar9);
  }
  plVar4 = local_68;
  uVar18 = local_78._8_8_;
  uVar17 = local_78._0_8_;
  plVar9[0xf] = 0;
  local_78 = (undefined1  [16])0x0;
  local_68 = (longlong *)0x0;
  if ((pcVar11 != (code *)0x0) &&
     (*(longlong *)pcVar11 = *(longlong *)pcVar11 + -1, *(longlong *)pcVar11 == 0)) {
    (**(code **)(*(longlong *)(pcVar11 + 8) + 0x30))(pcVar11);
  }
  if ((plVar10 != (longlong *)0x0) && (*plVar10 = *plVar10 + -1, *plVar10 == 0)) {
    (**(code **)(plVar10[1] + 0x30))(plVar10);
  }
  if ((plVar12 != (longlong *)0x0) && (*plVar12 = *plVar12 + -1, *plVar12 == 0)) {
    (**(code **)(plVar12[1] + 0x30))(plVar12);
  }
  if ((plVar13 != (longlong *)0x0) && (*plVar13 = *plVar13 + -1, *plVar13 == 0)) {
    (**(code **)(plVar13[1] + 0x30))(plVar13);
  }
  local_78 = CONCAT88(uVar18,uVar17);
  *plVar1 = *plVar1 + -1;
  local_68 = plVar4;
  if (*plVar1 == 0) {
    (**(code **)(plVar1[1] + 0x30))(plVar1);
  }
  *(longlong *)pcVar2 = *(longlong *)pcVar2 + -1;
  if (*(longlong *)pcVar2 == 0) {
    (**(code **)(*(longlong *)(pcVar2 + 8) + 0x30))(pcVar2);
  }
  FUN_23a33aa70(param_1,local_78._0_8_,local_78._8_8_,local_68);
  return (longlong *)0x0;
}
