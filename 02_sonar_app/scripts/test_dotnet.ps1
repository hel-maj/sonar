...
# Deterministic Common packages can share timestamps and file sizes across
# versions, so the UI gate must not trust incremental copy-local decisions.
Invoke-Dotnet @("build", $testProject, "--configuration", $Configuration, "--no-incremental", "--no-restore", "--nologo", $commonFeedProperty, $protocProperty, $packageCacheProperty, $developerProperty)
Invoke-Dotnet @(
    "run",
    "--project", $testProject,
