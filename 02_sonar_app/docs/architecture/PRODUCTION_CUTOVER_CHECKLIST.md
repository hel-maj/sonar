...
| `E11` | Memory readers: reeling, inventory, player status/chat evidence | **partial production** — the current signed-build profile composes the least-rights connector only for reeling regions. Player-status candidates are intentionally empty and `require_player_status=false`; inventory/status/chat evidence used elsewhere comes from separate visual owners, not an unproven memory claim | add and authenticate a build-specific player-status/chat profile before exposing those values; unknown/partial regions remain fail-closed |
