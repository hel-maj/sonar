...
| `E05` | Long-lived Engine process lifecycle, heartbeat, sequence, bounded event queues, reconnect/terminal cleanup | **done native** - production Host owns one Common Job-contained pair, bounded restart circuit and terminal cleanup; lifecycle start/stop races and controlled WPF failures have regression coverage | repeat sustained normal lifecycle and crash/recovery against signed release pair |
...
| `E09` | GTA process/window/client-area ownership and focus gate | **done native / live acceptance pending** — exact `GTA5.exe` resolver, least-rights generation snapshot and runtime target/foreground/client guard are production-composed | prove supported GTA build, focus loss and window-generation changes under the authorized live gate |
...
| `E11` | Memory readers: reeling, inventory, player status/chat evidence | **partial production** — the current signed-build profile production-composes least-rights reeling regions and coherent inventory open-state memory evidence; the latter is normalized through Common `unknown` / `closed` / `open`. Visual inventory evidence owns only item and context geometry. Player-status/chat candidates remain intentionally absent and `require_player_status=false` | add and authenticate a build-specific player-status/chat profile before exposing those values; unknown/partial regions remain fail-closed |
...
| `E15` | Inventory/menu/store-fish episode | **done native / live acceptance pending** — production composes the native observer, reflow-aware episode runner and E12 mutation lease | prove real inventory/context confirmation, target loss and cleanup |
...
| `E20` | Aggregate runtime events/audio/tackle/session feed | **partial production** — real production progress publishes revisioned session statistics and tackle through bounded queues; recent events retain 400/present 80 | route remaining typed safety/lifecycle events and complete the product audio adapter |
...
| `H05` | Hotkey capture/conflicts/suppress-until-release | **done native / live acceptance pending** - Common 0.2.18 owns capture/presentation, C#/C++ reject semantic conflicts, and normal Host owns one bounded Win32 polling lifecycle. It reads the latest persisted start/stop gesture, triggers the existing coarse entitlement-gated command once per press, suppresses while the product window is foreground and requires release before re-arming | authorized real key-state/reconfiguration/shutdown acceptance |
| `H06` | Telegram settings, availability, polling/notifications/commands | **production transport + native fishing actions composed / remaining actions partial** - licensed normal mode owns bounded long poll, reconfiguration, native fishing start/stop and current statistics/tackle projections. Missing player scan/screenshot/focus/shutdown adapters are hidden and fail closed | live Telegram acceptance; implement only real product-owned native adapters for the remaining actions |
...
| --- | --- | --- |
| `U00` | 8-item shell, branding, version/update/license regions, feature redirect, nested scroll policy | **done offline** — Common 0.2.18 AppShell/AppBrand, embedded Fishing logo, exact 8-item catalog, feature redirect, navigation footer and responsive page scroll are composed; production license/update sources remain separate rows |
...
| `U05` | Настройки behavior/depletion/thresholds/fish/hotkeys/uninstall | **partial** — state.dat atomic save, dependencies, 31-fish embedded selector, bulk actions, Common 0.2.18 styled inputs/wheel-safe combo (including `DisplayMemberPath` selection projection), threshold sliders and plain-letter hotkey conflict UX are composed; H08 provides an exact-confirmation safe uninstall plan, while after-exit execution remains pending |
...
The current local review artifact is `build/ui-gallery-0218-final/`; its
`gallery-manifest.json` records every rendered surface and DPI, pins Common UI
`0.2.18` and has SHA-256
...
regressions приняты. Первый live pass 2026-08-24 подтвердил target/hash-read и
capture, но exact profile fail-closed отклонил текущий build, поэтому memory не
читалась. Отдельный non-shipping
...
`pattern_scan_incomplete`. Registry не изменён, physical input не разрешён.
...
| Рыбалка | **working behind safety gates** — UI, global start/stop hotkey and Telegram start/stop call the same coarse native automation runtime | real target, foreground, supported build and current entitlement are mandatory; live side effects were not triggered by the offline gate |
...
| Стрим | **fail-closed prerequisite** — the page reports unavailable and mutation commands are disabled; no decorative start action is enabled | needs a redistributable encoder/tunnel payload, guarded capture, authenticated viewer/HLS transport, public HTTPS tunnel policy and chat bridge. See H07 |
