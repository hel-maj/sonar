...
| --- | --- | --- |
| `U00` | 8-item shell, branding, version/update/license regions, feature redirect, nested scroll policy | **done offline** - Common 0.2.21 AppShell/AppBrand, embedded Fishing logo, exact 8-item catalog, feature redirect, navigation footer and responsive page scroll are composed; production license/update sources remain separate rows |
...
| `U05` | Настройки behavior/depletion/thresholds/fish/hotkeys/uninstall | **partial** - state.dat atomic save, dependencies, 31-fish embedded selector, bulk actions, Common 0.2.21 styled inputs/wheel-safe combo (including `DisplayMemberPath` selection projection), threshold sliders and plain-letter hotkey conflict UX are composed; H08 provides an exact-confirmation safe uninstall plan, while after-exit execution remains pending |
...
Current source was rendered again after the exact Common UI 0.2.21 repin into
`build/ui-gallery-0221-final/`: 204 PNG cover every product page/variant at all
three layouts and four DPI targets. Its manifest pins `0.2.21` and has SHA-256
