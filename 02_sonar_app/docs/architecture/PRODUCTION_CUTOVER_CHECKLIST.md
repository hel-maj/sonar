...
| `H06` | Telegram settings, availability, polling/notifications/commands | **production transport + verified availability + native actions/events composed / remaining actions partial** - licensed normal mode owns real bounded `getMe` proof, long poll, reconfiguration, native fishing start/stop, current statistics/tackle and typed automatic catch/start-stop/meal/inventory/bait/focus notifications. Persisted runtime and draft probes are separate single-flight lanes: an unsaved draft cannot stop polling or reset its same-identity offset, while saving a different identity resets it once. Stale generation and late post-Stop results are dropped; Stop remains bounded for a non-cooperative adapter. New enable requires a matching fresh `Available`; a transient outage does not silently disable an already saved setting. Publisher remains bounded, generation/sequence-deduplicated and failure-isolated. Missing on-demand player scan/screenshot/focus/shutdown adapters are hidden and fail closed | live credential/network Telegram acceptance; implement only real product-owned native adapters for the remaining actions |
| `H07` | Streaming settings and FFmpeg/HLS/cloudflared lifecycle/chat mode | **Local Access composition done offline / live acceptance pending** - compile-isolated Local Access embeds hash-pinned FFmpeg 8.1.1 and cloudflared 2026.5.2, resolves one current GTA HWND, owns authenticated secret-path loopback HLS/viewer, launches both processes in Common kill-on-close Jobs and publishes a bounded HTTPS tunnel URL. The coarse revisioned controller retains exact 480/720/1080 + 30/10-FPS policy and bounded start/stop/retry/restart/autostop/cleanup. Ordinary licensed composition remains unavailable; absent chat bridge collapses its UI action | run real capture/network, DPI/resize/alt-tab, process-loss/restart, public-viewer and two-EXE lifecycle acceptance; settle third-party redistribution/signing for ordinary licensed composition and add the coarse chat use case before restoring its action; see [H07 runtime](H07_STREAMING_RUNTIME.md) |
| `H08` | Update/download/activation unit, uninstall and rollback | **done offline + local development maintenance / partial production activation** - signed-manifest/staging/swap/recovery core, exact allowlist and uninstall plan are green; a product-owned after-exit executor now performs explicit development-unsigned install/update/rollback/recovery and key-only legacy-license import with external backup, sanitized receipt and repeated no-Python gate. Product-facing uninstall stays collapsed until the production-signed activation owner is composed | production public key/metadata endpoint, signing certificate/timestamp, licensed composition, signed remote activation/uninstall and complete signed installed matrix; see [H08 transaction](H08_RELEASE_TRANSACTION.md) |
...
| `U05` | Настройки behavior/depletion/thresholds/fish/hotkeys | **done offline / live hotkey acceptance pending** - state.dat atomic save, dependencies, 31-fish embedded selector, bulk actions, Common 0.2.22 styled inputs/wheel-safe combo (including `DisplayMemberPath` selection projection), threshold sliders and plain-letter hotkey conflict UX are composed. Uninstall surface is intentionally absent until H08 production activation exists |
| `U06` | Статистика current-session metrics/table/prices/chart/reset | **done native / live values acceptance pending** - populated responsive Common DataGrid, embedded fish images, six metrics, chart and atomic custom-price revision are green. One append-only typed field-24 command resets the Engine-owned aggregate at a single linearization boundary; exact generation is required, reset is never replayed and one monotonic merge rejects a late response behind a newer event. Pre-issue timeout preserves the generation; ambiguous post-issue timeout/cancellation projects fail-closed empty. Completion observed by reset is reconciled once against the original Start. Failure is visible inline, success has no toast. History/CSV intentionally absent; see [ADR-0006](ADR-0006-CURRENT-SESSION-STATISTICS-RESET.md) |
| `U07` | Стрим page | **Local Access wired / live acceptance pending** - responsive Common page preserves status/area/quality/autostop/URL, quality/chat zoom/10-FPS settings and four metrics from one typed snapshot. Local Access receives the real embedded-tool/capture/HLS/tunnel controller when all exact resources are present; chat action is collapsed when the bridge capability is absent. Ordinary licensed composition still receives the fail-closed unavailable adapter | accept the Local Access process/network lifecycle and decide ordinary licensed payload policy; add chat only with a real coarse bridge |
| `U08` | Telegram page | **production transport / partial action parity** - Common composition, credential editors, real bounded `getMe` availability, settings, router/polling/planners and native fishing/statistics/tackle actions are wired. Unavailable OS/game actions are not advertised; live credentials/network remain acceptance |
