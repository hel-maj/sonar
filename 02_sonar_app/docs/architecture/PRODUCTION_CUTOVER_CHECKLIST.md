...
| `E11` | Memory readers: reeling, inventory, player status/chat evidence | **partial production** - coarse routing selects Fishing reeling-only for a positive same-frame trigger and exactly one Common CEF inventory-open facade capture otherwise, so cold discovery cannot delay A/D control. Unknown-only exponential retry is capped at 4 s; known state is never cached and generation change resets immediately. Local Access compile-time admits exact candidate 1.20.7, ordinary shipping remains denied/inert until promotion. Confirmed fish `1 -> 0` remains pending across one failed capture and is cleared only after coherent success. Screenshot owns item/context geometry, while menu/mini-game are independent same-frame visual facts. Player-status/chat production plans are absent | separately accept Local Access read-only behavior, promote a Common shipping profile only with independent evidence, then authenticate reeling and WebEngine fields; unknown/partial evidence remains fail-closed. See `RUNTIME_OBSERVATION_PARITY.md` |
...
| `E15` | Inventory/menu/store-fish episode | **partial production** - production composes the native observer, reflow-aware episode runner and E12 mutation lease. Local Access can obtain open-state through the exact Common candidate; ordinary shipping remains unknown by policy until promotion | prove Local Access read-only inventory/context confirmation, target loss and cleanup; keep shipping fail-closed until Common promotion |
...
| --- | --- | --- |
| `U00` | 8-item shell, branding, version/update/license regions, feature redirect, nested scroll policy | **done offline** - Common 0.2.19 AppShell/AppBrand, embedded Fishing logo, exact 8-item catalog, feature redirect, navigation footer and responsive page scroll are composed; production license/update sources remain separate rows |
...
| `U05` | Настройки behavior/depletion/thresholds/fish/hotkeys/uninstall | **partial** - state.dat atomic save, dependencies, 31-fish embedded selector, bulk actions, Common 0.2.19 styled inputs/wheel-safe combo (including `DisplayMemberPath` selection projection), threshold sliders and plain-letter hotkey conflict UX are composed; H08 provides an exact-confirmation safe uninstall plan, while after-exit execution remains pending |
...
The current local review artifact is `build/ui-gallery-0219-final/`; its
`gallery-manifest.json` records every rendered surface and DPI, pins Common UI
`0.2.19` and has SHA-256
