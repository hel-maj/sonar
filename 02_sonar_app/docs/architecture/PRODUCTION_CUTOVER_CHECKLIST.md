...
| `E11` | Memory readers: reeling, inventory, player status/chat evidence | **partial production** - reeling and inventory use separate memory scopes; inventory has typed bounded discovery/cache-revalidation/one-shot rediscovery and Common `unknown` / `closed` / `open` normalization. Shipping registry schema `2` explicitly has no admitted inventory binding. The actual current hash is not admitted and therefore returns typed `profile_unavailable` / `memory_game_build_unsupported`; the sole older admitted profile returns `memory_inventory_binding_unavailable`. Screenshot owns only item/context geometry. Player-status/chat candidates are also absent | run the prepared controlled `CLOSED/OPEN` characterization once, promote only a unique exact-current-build binding with regression/read-only acceptance, then separately authenticate reeling and player-status/chat for that build; unknown/partial evidence remains fail-closed |
...
| `E15` | Inventory/menu/store-fish episode | **partial production** - production composes the native observer, reflow-aware episode runner and E12 mutation lease, but unknown open-state blocks inventory-dependent mutations because no current-build binding is admitted | first close E11 inventory admission, then prove real inventory/context confirmation, target loss and cleanup |
