...
| Reeling geometry | `memory_reeling.py`: confirmed fish hash `802685111`, player/fish XYZ, player-right XY, derived distance | `reeling_evidence` с independent hash reread и bounded exact projections | Decoder/resolver parity покрыта на embedded offline historical baseline; это не current live admission, текущий hash не admitted |
...
| --- | --- | --- |
| Start fishing и reeling input | Visual fishing stage/targets того же кадра | Actual `GTA5.exe` SHA-256 `8c2c3f768b87f060d678d9e175842aa20449cf5bc164c630692a494eb353d472` отсутствует в registry; единственный live read-only compatibility pass завершился исторической объединённой причиной `pattern_scan_incomplete`, поэтому нет admitted E11 player/replay/fish evidence для final mutation gate |
