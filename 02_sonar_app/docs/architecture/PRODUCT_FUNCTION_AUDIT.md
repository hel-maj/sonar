...
| Catch/inventory/meal/bait/equipment/statistics | C++ production episodes + revisioned aggregate IPC; session statistics только в памяти | та же live authority; authentic UI/game evidence | Native code и fixtures есть. Inventory open/closed/unknown теперь приходит только из coherent memory через общий Common contract; screenshot владеет лишь item/context geometry. Live episode acceptance остаётся pending |
