...
9. Во время работы приложение повторяет ту же проверку каждые 5 минут. Если блокировка появилась уже после запуска, основной UI скрывается, активные runtime-задачи останавливаются, и показывается окно блокировки.

Endpoint является обязательным условием запуска и работы. Если endpoint недоступен, вернул HTTP-ошибку, прислал невалидный JSON или подпись не прошла проверку, проверка считается непрошедшей. В этом случае основной интерфейс не открывается или скрывается во время работы, а приложение показывает окно обновления.
