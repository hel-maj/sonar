...
## Диагностика OCR описаний предметов

Для проверки обрезки и чтения описаний предметов есть отдельная папка `test_info_ocr`.
...
Скрипт не дублирует OCR-логику. Он напрямую использует `sonar.fishing.item_info.ItemInfoDetector`, поэтому проверяет ту же обрезку и чтение, которые использует бот при наведении на предмет перед употреблением.
...
Перед запуском положи полноэкранные скриншоты инвентаря в:

```text
test_info_ocr/screenshots
```

После запуска результаты появятся в:

```text
test_info_ocr/results
```

Что создается:

- `results/slices` - нарезанные области описаний предметов;
- `results/results.csv` - табличный результат OCR и парсинга;
- `results/results.json` - полный результат в JSON;
- `results/results.txt` - быстрый текстовый просмотр результата.

Используй этот скрипт, когда надо проверить, что бот реально найдет на новых скриншотах, без запуска игры и без копирования кода обрезки/OCR в отдельный тестовый файл.
